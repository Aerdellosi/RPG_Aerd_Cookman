import {Character, Items, ItemObj, Enemy, listOfFirstItemsArray} from './../src/section5RPG.js';
// FireLizard, RockMonster, Wizard, Skeleton, Wolves, Pirate, Bats, ShadowDemon, SpiderSnake, CharClass
describe('Character', () => {
...
describe("Enemy", () => {
  ...
  test("this should roll to see if a player has hit", () => {
    let dummyList = ["blunderbuss", "katana", "gun kit"];
    let enemy = new Enemy(70, 10, 5, 10, 4, 35);
    let rickybob = new Character("missionary", dummyList);
    let hpCheck = rickybob.hp;
    hpCheck = enemy.attack(rickybob.dodgeChance, hpCheck);
    expect(hpCheck).toEqual(11);
  });

});

describe("listOfFirstItemsArray", () => {

  test("should make an array of the first selectable items.", () => {

    let first = listOfFirstItemsArray.apply[0];
    expect(first).toEqual("cavalry sabre", 6, 10, "A Common seafaring weapon. 6 damage, 10 range");
  })
})
